package org.firstinspires.ftc.teamcode.RoverRuckus.externalLib;
 ...
 *
 * @see
 * <a href="http://brettbeauregard.com/blog/2011/04/improving-the-beginners-pid-direction/improving-the-beginners-pid-introduction">pid</a>
 */
...
public class MiniPID {
	 ...
	 *          Small values prevents P and I terms from
	 *          causing overshoot.
	 */
	 ...
	 *          Small values prevents P and I terms from
	 *          causing overshoot.
	 * @param f Feed-forward gain. Open loop "best guess" for the output
	 *          should be. Only useful if setpoint represents
	 *          a rate.
	 */
	 ...
	 * with only PI.<br>
	 * For position based controllers, this is the first parameter to tune,
	 * with I second. <br>
	 * For rate controlled systems, this is often the second after F.
	 *
	 * @param p Proportional gain. Affects output according to <b>output+=P*
	 *          (setpoint-current_value)</b>
	 */
	 ...
	 * Typically tuned second for "Position" based modes, and third for "Rate"
	 * or continuous based modes. <br>
	...
	public void setI(double i) {
		...
		// As an example doubling the I term cuts the accumulated error in
	 ...
	 * <list>
	 * <li> Adds a "startup kick" and speeds up system response during
	 * setpoint changes
	 ...
	 * However, in many systems a large D value will cause significant
	...
	/**
	 * Set the maximum output value contributed by the I component of the
	 * system
	...
	public void setMaxIOutput(double maximum) {
		...
		// against
		// the max error are far more common than changing the I term or
		// Izone.
		maxIOutput = maximum;
		if (I != 0) {
			maxError = maxIOutput / I;
	...
	public double getOutput(double actual, double setpoint) {
		...
		// overshoot
		Doutput = -D * (actual - lastActual) / elapsedTime;
		lastActual = actual;
		...
		// Iterm.
		// 2. prevent windup by not increasing errorSum if we're already
		// running against our max Ioutput
		// 3. prevent windup by not increasing errorSum if output is
		// output=maxOutput
		Ioutput = I * errorSum * elapsedTime;
		if (maxIOutput != 0) {
		...
		if (minOutput != maxOutput && !bounded(output, minOutput, maxOutput)) {
			...
			// In addition to output limiting directly, we also want to
			// prevent I term
		...
		// System.out.printf("Final output %5.2f [ %5.2f, %5.2f , %5.2f  ],
	 ...
	 * This should be used any time the PID is disabled or inactive for
	 * extended
	 * duration, and the controlled portion of the system may have changed
	 * due to
	 * external forces.
	 */
	public void reset() {
	 ...
	 * <br>Can simplify tuning by helping tuning over a small range applies to
	 * a much larger range.
	 * <br>This limits the reactivity of P term, and restricts impact of large
	 * D term
	 * during large setpoint adjustments. Increases lag and I term if range is
	 * too small.
	 *
	 ...
	 * Uses an exponential weighted rolling sum filter, according to a simple
	 * <br>
