...
public class AutoMoveControllerTest extends OurLinearOpMode {
	...
	protected void initialize() throws InterruptedException {
		...
		autoMoveController = new AutoMoveController(robot, new PositionTracker(),
		                                            rampedMoveController);
		manualMoveController =
				new ManualMoveController(
						robot, rampedMoveController);
		robot.initIMU();
	...
	protected void run() {
		...
		while (opModeIsActive()) {
			if (a.pressed()) autoMoveController.addToTargetLocation(FORWARD);
			if (b.pressed()) autoMoveController.addToTargetLocation(BACKWARD);
			if (x.pressed()) autoMoveController.addToTargetAngle(Math.toRadians(90));
			if (y.pressed()) autoMoveController.reset();
